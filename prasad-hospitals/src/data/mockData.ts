interface Branch { id: string; name: string; area: string; address: string; phone: string; googleMapsReviewUrl: string; }
...
interface Slot { id: string; doctorId: string; date: string; time: string; session: 'morning' | 'afternoon' | 'evening'; available: boolean; }
  ...
  for (let d = 0; d < 3; d++) {
    ...
    sessions.forEach(({ session, times }) => {
      times.forEach((time, i) => {
        ...
        slots.push({ id: `${doctorId}-${dateStr}-${session}-${i}`, doctorId, date: dateStr, time, session, available: idx % 3 !== 2 });
      });
    });
  }
  return slots;
...
export const branches: Branch[] = [
  { id: 'nacharam', name: 'Prasad Hospitals Nacharam', area: 'Nacharam, Hyderabad', address: '#3-6-289, Nacharam Road, Nacharam, Hyderabad – 500076', phone: '+91-40-2304-5678', googleMapsReviewUrl: 'https://search.google.com/local/writereview?placeid=ChIJ6QM34QWRyzsRcgkV5-ydfWI' },
  { id: 'pragathi-nagar', name: 'Prasad Hospitals Pragathi Nagar', area: 'Pragathi Nagar, Hyderabad', address: '#2-1-150, Pragathi Nagar, Kukatpally, Hyderabad – 500090', phone: '+91-40-2784-5678', googleMapsReviewUrl: 'https://search.google.com/local/writereview?placeid=PLACEHOLDER_pragathi_nagar' },
  { id: 'manikonda', name: 'Prasad Hospitals Manikonda', area: 'Manikonda, Hyderabad', address: 'Plot No. 47, Manikonda Main Road, Manikonda, Hyderabad – 500089', phone: '+91-40-6674-5678', googleMapsReviewUrl: 'https://search.google.com/local/writereview?placeid=PLACEHOLDER_manikonda' },
];
...
const doctorTemplates: Record<string, { suffix: string; name: string; qualification: string; nextAvailable: string }[]> = {
  cardiology: [
...
export const doctors: Doctor[] = departments.flatMap(dept => {
  ...
  return templates.map(t => ({
    id: `${dept.id}-${t.suffix}`, departmentId: dept.id, name: t.name, qualification: t.qualification,
...
export const slots: Slot[] = doctors.flatMap(doctor => generateSlots(doctor.id, TODAY));

export function getDepartmentsByBranch(branchId: string): Department[] { return departments.filter(d => d.branchId === branchId); }
export function getDoctorsByDepartment(departmentId: string): Doctor[] { return doctors.filter(d => d.departmentId === departmentId); }
export function getSlotsByDoctor(doctorId: string): Slot[] { return slots.filter(s => s.doctorId === doctorId); }
